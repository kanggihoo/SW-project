...
class State(TypedDict):
    ...
    user_message: str
    intent: str
    product_id: str  # 명시적 상품 ID

    # --- 정보 수집/업데이트 관련 ---
    cloth_search: ClothSearch  # TPO, 색상, 스타일 등
...
- **`master_router`**: 가장 먼저 실행됩니다.
    1. `product_id`가 있으면 → `product_info_agent`로 직접 라우팅.
    2. 없으면 → `intent_classify_node`로 보내 의도 분석 시작.
...
- **기타 노드**:
    ...
    - `info_qa_node`: 웹 검색 도구를 사용하여 패션 트렌드, 용어 등 정보성 질문에 답변.
...
- **`prepare_search_cycle_node`**: **새로운 검색**을 준비합니다.
    ...
    - 변경된 조건에 해당하는 전문가(들)를 `experts_to_run` 리스트에 추가. (예: `style` 변경 시 `style_analyst`만 실행)
...
- **`prepare_cache_cycle_node`**: **캐시된 결과를 보여주기 전** 준비합니다.
    ...
    - **하나의 전문가라도 보여줄 코디 세트가 없으면, 전체 캐시 순환은 중단됩니다.**

- **`external_llm_node` (전문가 분석)**: `current_expert`의 관점에서 사용자 요청을 분석하여 구체적인 검색 키워드(의견)를 생성하고 `expert_opinions`에 저장합니다.
...
- **`decide_work_after_pop`**: 전문가 루프 내에서 작업을 결정합니다.
    ...
    - 없으면 → `external_llm_node` (새로운 전문가 분석)
...
6.  **전문가 루프 (x3)**: 각 전문가는 `external_llm_node`에서 분석 의견을 만들고, `search_node`에서 벡터 검색을 수행하여 상/하의 코디 세트를 `expert_search_cache`에 저장합니다.
