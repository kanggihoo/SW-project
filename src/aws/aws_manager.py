...
class AWSManager:
    ...
    def _initialize_services(self) -> None:
        try:
            ...
            self.s3_manager = S3Manager(region_name=s3_config.get('region_name'), bucket_name=s3_config.get('bucket_name'))

            # dynamodb 초기화
            dynamodb_config = self.config.get_dynamodb_config()
            self.dynamodb_manager = DynamoDBManager(
                region_name=dynamodb_config.get('region_name'), table_name=dynamodb_config.get('table_name'), config=dynamodb_config
            )
    ...
    def _test_connection(self):
        ...
        return {'success': all([s3_success, dynamodb_success]), 's3': s3_success, 'dynamodb': dynamodb_success}
    ...
    def get_product_images_from_paginator(self, item: dict) -> list[ImageManager]:
        ...
        product_model = ProductManager(
            main_category=item['main_category'], product_id=item['product_id'], sub_category=item['sub_category'], images=images
        )
