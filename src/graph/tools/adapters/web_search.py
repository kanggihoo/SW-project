...
def format_search_result(result: dict, include_raw_content: bool = False) -> str:
    ...
    raw_content = ''
    if include_raw_content and 'raw_content' in result and result['raw_content'] is not None and len(result['raw_content'].strip()) > 0:
...
class TavilySearch(BaseTool):
        ...
        **kwargs,
        ...
        if days is not None:
            if params['topic'] == 'general':
                print("Warning: days parameter is ignored for 'general' topic search. Set topic parameter to 'news' to use days.")
            else:
                params['days'] = days
        ...
        # 결과 포맷팅
        format_output = format_output if format_output is not None else self.format_output
        if format_output:
    ...
    def get_search_context(
        ...
        **kwargs,
        ...
        if format_output:
            context = [format_search_result(source, include_raw_content=False) for source in sources]
        else:
            context = [
                {
                    'url': source['url'],
