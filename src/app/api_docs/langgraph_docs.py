...
def sse_response_example() -> dict[int, Any]:
    ...
    message_example = {
        ...
        'content': {
            'type': 'ai',
            'content': '네이비 베이직 반팔 셔츠에 아이보리 와이드 슬랙스가 잘 어울려! 네이비와 아이보리는 명도 대비를 통해 세련된 균형감을 만들어내고, 차가운 네이비와 따뜻한 아이보리의 색상 온도 대비로 시각적 긴장감을 형성해. 블랙 로퍼와 블랙 가방으로 포인트를 더하면서 전체적인 색상 밸런스를 완성할 수 있어.',
            'tool_calls': [],
            'tool_call_id': None,
            'additional_kwargs': {
                'type': 'refer',
                'created_at': '2025-09-25T08:51:48.710757+00:00',
                ...
                'text/event-stream': {
                    ...
                    'examples': {
                        ...
                        '메시지 이벤트': {
                            ...
                            'description': "하나의 에이전트 작업이 완료된 후, 완전하고 구조화된 최종 결과물을 클라이언트에 전달할 때 사용됩니다. `content`는 `ChatMessage` 객체이며 다음을 포함합니다:\n- `type`: 메시지 유형으로, 보통 'ai'입니다.\n- `content`: AI가 생성한 최종 텍스트 응답입니다.\n- `additional_kwargs`: AI 응답에 대한 추가 정보를 담는 객체입니다. 내부 `type` 필드에 따라 구조가 달라질 수 있습니다.\n  - `type`: 추가 정보의 종류를 나타냅니다. 예시에서는 `'refer'`(상품 참조)이지만, `'image'`(이미지 URL) 등 다른 타입이 올 수 있습니다.\n  - `created_at`: 메시지 생성 타임스탬프입니다.\n  - `expert_type`: 응답을 생성한 전문가의 유형입니다 (예: 'style_analyst', 'color_expert').\n  - `product_ids`: `type`이 `'refer'`일 경우, AI가 참조한 상품의 ID 목록입니다.\n  - `image_urls`: `type`이 `'image'`일 경우, 관련된 이미지의 URL 목록입니다.",
...
ERROR_RESPONSES = {500: {'description': '서버 내부 오류가 발생했습니다', 'model': ErrorResponse}}
...
data: {"type": "message", "content": {"type": "ai", "content": "네이비 베이직 반팔 셔츠에 아이보리 와이드 슬랙스가 잘 어울려! 네이비와 아이보리는 명도 대비를 통해 세련된 균형감을 만들어내고, 차가운 네이비와 따뜻한 아이보리의 색상 온도 대비로 시각적 긴장감을 형성해. 블랙 로퍼와 블랙 가방으로 포인트를 더하면서 전체적인 색상 밸런스를 완성할 수 있어.", "tool_calls": [], "tool_call_id": null, "additional_kwargs": {"type": "refer", "created_at": "2025-09-25T08:51:48.710757+00:00", "expert_type": "color_expert", "product_ids": ["3858441_블루", "4109513_화이트"]}}}
...
data: {"type": "message", "content": {"type": "ai", "content": "화이트 셔츠에 블랙 와이드 슬랙스가 잘 어울려. 데이트 분위기에 맞게 소매는 자연스럽게 내리고 셔츠 단추 위쪽 1-2개만 살짝 풀어주는 게 좋아. 셔츠는 전체적으로 바지 안에 넣어서 깔끔한 실루엣을 만들면서, 블랙 벨트로 허리선을 살짝 강조해봐. 블랙 클래식 로퍼랑 매치하면 세련된 데이트룩이 완성될 거야. 캐주얼하면서도 포멀한 느낌이 적절히 섞여서 특별한 날에도 손색없을 만한 스타일이야.", "tool_calls": [], "tool_call_id": null, "additional_kwargs": {"type": "refer", "created_at": "2025-09-25T08:51:57.832308+00:00", "expert_type": "style_analyst", "product_ids": ["2171532_화이트", "2503135_블랙"]}}}
