...
class MusinsaAPIWrapper:
    ...
    async def get_size_recommend(self, product_id: str | int, height: str | int, weight: str | int) -> dict:
        ...
        try:
            ...
            if raw_data.get("data", {}).get("sizeRecommends"):
                ...
                return {"success": False, "data": [], "message": "해당 조건에 맞는 사이즈 추천 데이터가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except httpx.HTTPStatusError as e:
            return {"success": False, "data": [], "message": f"API 서버 에러 (상태 코드: {e.response.status_code})", "error_details": {**error_context, "error_type": ErrorType.HTTP_ERROR, "status_code": e.response.status_code}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"알 수 없는 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.UNKNOWN}}
    ...
    async def get_product_selection_info(self, product_id: str | int) -> Dict[str, Any]:
        ...
        try:
            ...
            if filter_option and filter_option.get("optionCount", 0) > 0:
                ...
                return {"success": False, "data": [], "message": f"제품(ID: {product_id})에 대한 선택 옵션 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_OPTIONS}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"제품 선택 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
            ...
            if raw_data.get("data"):
                ...
                for option_filter in sorted(data.get("basic", []), key=lambda x: x["sequence"]):
                    option_filters.append({
                        ...
                        "values": [{"id": val.get("no"), "name": val.get("name"), "code": val.get("code")} for val in option_filter.get("optionValues", [])]
                    })
                stock_by_options = []
                ...
                result_data = [{"product_id": product_id_str, "option_count": len(option_filters), "option_filters": option_filters, "stock_by_options": stock_by_options}]
                return {"success": True, "data": result_data, "message": "제품 옵션 및 재고 정보를 성공적으로 조회했습니다."}
            else:
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 옵션 정보를 찾을 수 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"옵션 및 재고 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
        ...
        try:
            ...
            if raw_data.get("data") and raw_data["data"].get("sizes"):
                ...
                for size in data.get("sizes", []):
                    size_details.append({"size_name": size.get("name"), "items": [{"name": item.get("name"), "value": item.get("value")} for item in size.get("items", [])]})
                result_data = [{"product_id": product_id_str, "size_guide_image_url": "https:" + data["webImage"] if data.get("webImage") else "", "size_details": size_details}]
                return {"success": True, "data": result_data, "message": "제품 실측 사이즈 정보를 성공적으로 조회했습니다."}
            else:
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 실측 사이즈 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"실측 사이즈 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_review_summary(self, product_id: str | int) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_data.get("data") and raw_data.get("meta", {}).get("result") == "SUCCESS":
                data = raw_data["data"]
                result_data = [{"product_id": product_id_str, "total_review_count": data.get("totalCount", 0), "style_review_count": data.get("styleCount", 0), "monthly_review_count": data.get("monthCount", 0), "general_review_count": data.get("generalCount", 0) + data.get("photoCount", 0) + data.get("goodsCount", 0), "average_rating": data.get("satisfactionScore", 0.0)}]
                return {"success": True, "data": result_data, "message": "리뷰 요약 정보를 성공적으로 조회했습니다."}
            else:
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 리뷰 요약 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"리뷰 요약 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_filtered_review_count(self, 
        ...
        params = {
            "goodsNo": product_id_str, "hasPhoto": has_photo, "option1List": option_list, 
            "sex": sex , "selectedSimilarNo": product_id_str
        }
        ...
        try:
            ...
            if raw_data.get("meta", {}).get("result") == "SUCCESS" and "data" in raw_data:
                ...
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 리뷰 개수 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"리뷰 개수 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
        ...
        try:
            ...
            if raw_data.get("data", {}).get("list"):
                ...
                for review in raw_data["data"]["list"]:
                    ...
                    review_list.append({
                        "id": review.get("no"), "content": review.get("content"), "rating": review.get("grade"),
                        "goods_option": review.get("goodsOption"), "created_at": review.get("createDate"),
                        "like_count": review.get("likeCount"),
                        "user_info": {"level": user_profile.get("userLevel"), "sex": user_profile.get("reviewSex"), "height_cm": user_profile.get("userHeight"), "weight_kg": user_profile.get("userWeight")}
                    })
                return {"success": True, "data": review_list, "message": "리뷰 목록을 성공적으로 조회했습니다."}
            else:
                return {"success": False, "data": [], "message": "해당 조건에 맞는 리뷰가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"리뷰 목록 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_product_like_count(self, product_id: Union[str, int, List[Union[str, int]]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_data.get("data", {}).get("success", False):
                ...
                return {"success": False, "data": [], "message": f"제품(ID: {product_ids})의 '좋아요' 정보를 찾을 수 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"'좋아요' 수 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_product_stats(self, product_id: str | int) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_data.get("data") and raw_data.get("meta", {}).get("result") == "SUCCESS":
                ...
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 통계 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"제품 통계 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_product_other_color(self, product_id: str | int) -> Dict[str, Any]:
        ...
        try:
                ...
                for tab in raw_data["data"]["curationTabs"]:
                    if tab.get("curationType") == "OTHER_COLOR":
                        for item in tab.get("curationGoodsList", []):
                            other_color_products.append({"product_id": item.get("goodsNo"), "goods_name": item.get("goodsName"), "image_url": item.get("imageUrl"), "is_sold_out": item.get("isSoldOut")})
                        break
            if other_color_products:
                ...
                return {"success": False, "data": [], "message": f"제품(ID: {product_id_str})의 다른 색상 제품 정보가 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"다른 색상 제품 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
        ...
        try:
            ...
            if match:
                ...
                try:
                    ...
                    result_data = [{"brand_info": {"brand_name": product_state_dict["brandInfo"]["brandName"], "brand_english_name": product_state_dict["brandInfo"]["brandEnglishName"].lower()}, "price_info": {"sale_price": product_state_dict["goodsPrice"]["salePrice"], "original_price": product_state_dict["goodsPrice"]["normalPrice"], "discount_rate": product_state_dict["goodsPrice"]["discountRate"], "is_on_sale": product_state_dict["goodsPrice"]["isSale"]}}]
                    return {"success": True, "data": result_data, "message": "제품의 브랜드 및 가격 정보를 성공적으로 조회했습니다."}
                except json.JSONDecodeError:
                    return {"success": False, "data": [], "message": "웹 페이지 내 데이터의 JSON 형식이 올바르지 않습니다.", "error_details": {**error_context, "error_type": ErrorType.JSON_PARSING_FAILED}}
            else:
                return {"success": False, "data": [], "message": "웹 페이지에서 제품 정보를 추출하는데 실패했습니다.", "error_details": {**error_context, "error_type": ErrorType.REGEX_MATCH_FAILED}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"제품 정보 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
    ...
    async def get_brand_likes_count(self, brand_name: Union[str, List[str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_data.get("data", {}).get("success", False):
                ...
                return {"success": False, "data": [], "message": f"브랜드(이름: {brand_names})의 '좋아요' 정보를 찾을 수 없습니다.", "error_details": {**error_context, "error_type": ErrorType.NO_DATA, "reason": raw_data.get("error", {}).get("message")}}
        except Exception as e:
            return {"success": False, "data": [], "message": f"브랜드 '좋아요' 수 조회 중 오류가 발생했습니다: {str(e)}", "error_details": {**error_context, "error_type": ErrorType.API_ERROR}}
