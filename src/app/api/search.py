...
async def search_product(
    ...
    product_id: Annotated[
        str,
        Path(description='조회할 상품 sku_id', example='4149670_데님', regex=PRODUCT_ID_REGEX),
    ],
):
    ...
    try:
        tasks = [
            ...
            musinsa_api_wrapper.get_product_brand_and_price(http_client, splited_product_id),
        ]
        ...
        suceess = brand_and_price.get('success')
        ...
        if suceess:
            current_price = brand_and_price.get('data')[0].get('price_info', {}).get('sale_price')
            original_price = brand_and_price.get('data')[0].get('price_info', {}).get('original_price')
            discount_rate = brand_and_price.get('data')[0].get('price_info', {}).get('discount_rate')
            is_on_sale = brand_and_price.get('data')[0].get('price_info', {}).get('is_on_sale')
        else:
            logger.warning(f'Musinsa API Wrapper get_product_brand_and_price failed: {brand_and_price}')
