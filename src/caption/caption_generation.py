...
async def process_single_item(item: dict, dep: CaptionDependency):
    try:
        ...
        # 다운로드 된 이미지 파싱
        base64_data_for_llm = await asyncio.to_thread(parsing_data_for_llm, images, image_sizes=dep.size)
...
async def main():
    ...
    try:
        ...
        """
        pagenator = dep.aws_manager.dynamodb_manager.get_product_pagenator(
            partition={'key': 'curation_caption_status', 'value': 'COMPLETED#COMPLETED', 'type': 'S'},
