...
class FashionCaptionGenerator:
    ...
    def _init(self, config: Config = None):
        ...
        if self.config.get('DEFAULT_TRACING_ENABLED'):
            setup_langsmith_tracing(
                enable_tracing=self.config.get('DEFAULT_TRACING_ENABLED'), project_name=self.config.get('DEFAULT_LANGCHAIN_PROJECT_NAME')
            )
        else:
    ...
    async def ainvoke(
        self, base64_data_for_llm: Base64DataForLLM, category: str, has_size: bool = True, config: RunnableConfig = None
    ) -> Dict[str, Any]:
        """상품 이미지 분석 실행"""
        llm_input = {
