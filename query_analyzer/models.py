...
class BottomFilter(BaseModel):
    ...
    주의: 쿼리에 명시적으로 언급된 정보만 추출하며, 유추는 절대 금지됩니다.
    """
    sub_category: Annotated[BottomSubCategory, Field(
    ...
    pattern_type: Annotated[BottomPatternType, Field(
        description="""사용자 쿼리에서 언급된 패턴 타입.
                   - 가능한 값: '무지', '스트라이프', '체크', '카모플라쥬', '워싱 (데님 전용)', '기타 패턴'.
                   - 쿼리에 해당 정보가 없으면 None으로 설정."""
    )]
    length: Annotated[BottomLengthType, Field(
...
class SingleCallAnalysisResult(BaseModel):
    ...
    analyzed_items: list[Union[TopFilter, BottomFilter]] = Field(
        ...,
        description="사용자 쿼리에서 식별된 모든 의류 아이템의 분석 결과 리스트"
    )
