...
class ProductMetaManager:
    ...
    def show_terminal(self):
        ...
        pipeline = [
            {
                '$group': {
                    '_id': {'main_category': '$main_category', 'sub_category': '$sub_category', 'data_status': '$data_status'},
                    'total_count_per_status': {'$sum': 1},
                }
            },
            {
                '$group': {
                    '_id': {'main_category': '$_id.main_category', 'sub_category': '$_id.sub_category'},
                    'status_breakdown': {'$push': {'status': '$_id.data_status', 'count': '$total_count_per_status'}},
            ...
            {
                '$group': {
                    '_id': {'main_category': '$main_category', 'sub_category': '$sub_category'},
                    'CR_DET': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'CR_DET']}, '$status_breakdown.count', 0]}},
                    'AWS_UPL': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'AWS_UPL']}, '$status_breakdown.count', 0]}},
                    'RE_COMP': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'RE_COMP']}, '$status_breakdown.count', 0]}},
                    'CA_COMP': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'CA_COMP']}, '$status_breakdown.count', 0]}},
                    'EB_COMP': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'EB_COMP']}, '$status_breakdown.count', 0]}},
                    'CL_COMP': {'$sum': {'$cond': [{'$eq': ['$status_breakdown.status', 'CL_COMP']}, '$status_breakdown.count', 0]}},
    ...
    def show_df(self):
        pipeline = [
            ...
            },
            {
                '$group': {
                    '_id': {'main_category': '$_id.main_category', 'sub_category': '$_id.sub_category'},
                    'status_list': {'$push': {'status': '$_id.data_status', 'count': '$total_count'}},
                    'total_count': {'$sum': '$total_count'},
                }
            },
            {'$project': {'_id': 0, 'main_category': '$_id.main_category', 'sub_category': '$_id.sub_category', 'status_list': 1, 'total_count': 1}},
        ]
