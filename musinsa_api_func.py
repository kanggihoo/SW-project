                ...
                selection_data = [
                    {
                        ...
                        'primary_options': [{'item_id': opt.get('itemNo'), 'name': opt.get('name')} for opt in filter_option.get('firstOptions', [])],
                        'secondary_options': [
...
async def get_filtered_review_count(
    ...
    async with httpx.AsyncClient() as client:
        try:
            params = {'goodsNo': product_id_str, 'hasPhoto': has_photo, 'option1List': option_list, 'sex': sex}
