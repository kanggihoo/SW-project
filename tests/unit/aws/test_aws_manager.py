...
class TestAWSManagerWithMocks:
    ...
    def test_test_connection_success(self, mock_aws_manager_initialized):
        """연결 테스트 성공 시나리오"""
        ...
        assert result['dynamodb'] is True
    
        manager.s3_manager.test_connection.assert_called_once()
        manager.dynamodb_manager.test_connection.assert_called_once()
    
    def test_initialization_success(self, mock_aws_manager_initialized):
        """AWSManager 정상 초기화 테스트"""
        manager = mock_aws_manager_initialized
        
        assert manager is not None
        assert manager.region_name == "ap-northeast-2"
        assert manager.bucket_name == "test-sw-fashion-image-data"
        assert manager.table_name == "TestProductAssets"
        assert hasattr(manager, 's3_manager')
        assert hasattr(manager, 'dynamodb_manager')
        
        logger.info("AWSManager successfully initialized with mocks")
    
    def test_initialization_failure_s3_connection(self, aws_mock_config, mock_s3_manager_connection_failed, mock_dynamodb_manager):
        """S3 연결 실패 시 초기화 오류 테스트"""
        with patch.object(AWSManager, '_load_config', return_value=aws_mock_config), \
             patch('aws.aws_manager.S3Manager', return_value=mock_s3_manager_connection_failed), \
             patch('aws.aws_manager.DynamoDBManager', return_value=mock_dynamodb_manager):
            
            with pytest.raises(ConnectionError) as exc_info:
                AWSManager()
            
            assert "S3 연결 실패" in str(exc_info.value)
            logger.info("S3 connection failure handled correctly")
    
    def test_initialization_failure_dynamodb_connection(self, aws_mock_config, mock_s3_manager, mock_dynamodb_manager_connection_failed):
        """DynamoDB 연결 실패 시 초기화 오류 테스트"""
        with patch.object(AWSManager, '_load_config', return_value=aws_mock_config), \
             patch('aws.aws_manager.S3Manager', return_value=mock_s3_manager), \
             patch('aws.aws_manager.DynamoDBManager', return_value=mock_dynamodb_manager_connection_failed):
            
            with pytest.raises(ConnectionError) as exc_info:
                AWSManager()
            
            assert "DynamoDB 연결 실패" in str(exc_info.value)
            logger.info("DynamoDB connection failure handled correctly")
    
    def test_initialization_failure_both_connections(self, aws_mock_config, mock_s3_manager_connection_failed, mock_dynamodb_manager_connection_failed):
        """S3와 DynamoDB 모두 연결 실패 시 테스트"""
        with patch.object(AWSManager, '_load_config', return_value=aws_mock_config), \
             patch('aws.aws_manager.S3Manager', return_value=mock_s3_manager_connection_failed), \
             patch('aws.aws_manager.DynamoDBManager', return_value=mock_dynamodb_manager_connection_failed):
            ...
            assert "S3 연결 실패" in error_msg
            assert "DynamoDB 연결 실패" in error_msg
            logger.info("Both connections failure handled correctly")
