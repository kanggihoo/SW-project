...
data: {"type": "status", "content": {"task_id": "color_expert", "state": "progress", "content": "최종 착장 선택: 4184e4bcd92438fd92e20fabbd9f7da8", "error_details": null}}
...
data: {"type": "token", "content": "이트 베이"}

data: {"type": "token", "content": "체적인 "}

data: {"type": "token", "content": "밸런스를 완"}

data: {"type": "token", "content": "성해"}
...
async def test_build_external_llm_graph(mock_llm_call, test_state, test_config):
    logger.remove()
    logger.add(
        sink=sys.stderr,
        level='INFO',
