...
def aws_mock_config():
    ...
    return {
        'aws': {'region_name': 'ap-northeast-2'},
        's3': {'bucket_name': 'test-sw-fashion-image-data'},
        'dynamodb': {'table_name': 'TestProductAssets'},
        'DEFAULT_PROJECTION_FIELDS': ['sub_category', 'product_id', 'curation_status', 'main_category', 'representative_assets'],
        'DEFAULT_PAGINATOR_CONFIG': {'PageSize': 10, 'MaxItems': 100},
    }
...
def sample_dynamodb_item():
    ...
    return {
        ...
        'curation_status': {'S': 'COMPLETED'},
        'representative_assets': {
            'M': {'front': {'S': 'segment/test_front.jpg'}, 'back': {'S': 'segment/test_back.jpg'}, 'model': {'S': 'segment/test_model.jpg'}}
        },
        'text': {'L': [{'S': 'test_text1.jpg'}, {'S': 'test_text2.jpg'}]},
    }
...
def mock_dynamodb_manager_initialized(aws_mock_config, mock_dynamodb_client):
    ...
    with (
        patch('aws.dynamodb.boto3.client', return_value=mock_dynamodb_client),
...
def mock_s3_manager_initialized(aws_mock_config, mock_s3_client):
    ...
    with patch('aws.s3.boto3.client', return_value=mock_s3_client), patch.object(S3Manager, '_load_config', return_value=aws_mock_config):
        manager = S3Manager()
        return manager
